...
#pragma HLS INLINE
 ...
 (void) ((!!(height<1080)) || (_assert("height<MAX_IMG_ROWS","Sobel/Sobel.cpp",37),0));
    (void) ((!!(width<1920)) || (_assert("width<MAX_IMG_COLS","Sobel/Sobel.cpp",38),0));
    (void) ((!!(vconv_xlim<1920 -(K-1))) || (_assert("vconv_xlim<MAX_IMG_COLS-(K-1)","Sobel/Sobel.cpp",39),0));
...
#pragma HLS PIPELINE
 T in_val=hconv.read();
    ...
    Border:for(int i=0;i<height;i++){
        VITIS_LOOP_75_1: for(int j=0;j<width;j++){
...
__attribute__((sdx_kernel("Sobel", 0))) void Sobel(video_stream &in_data,video_stream& out_data,
  int w, int h) {
