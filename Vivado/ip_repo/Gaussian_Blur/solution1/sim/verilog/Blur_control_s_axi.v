...
// 0x24 : Data signal of sigma
//        bit 31~0 - sigma[63:32] (Read/Write)
// 0x28 : reserved
// 0x2c : Data signal of kernal
//        bit 31~0 - kernal[31:0] (Read/Write)
// 0x30 : reserved
...
localparam
    ...
    ADDR_SIGMA_DATA_1  = 6'h24,
    ADDR_SIGMA_CTRL    = 6'h28,
    ADDR_KERNAL_DATA_0 = 6'h2c,
    ADDR_KERNAL_CTRL   = 6'h30,
    WRIDLE             = 2'd0,
    WRDATA             = 2'd1,
    WRRESP             = 2'd2,
    WRRESET            = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_KERNAL_DATA_0: begin
                    rdata <= int_kernal[31:0];
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KERNAL_DATA_0)
            int_kernal[31:0] <= (WDATA[31:0] & wmask) | (int_kernal[31:0] & ~wmask);
    end
end
