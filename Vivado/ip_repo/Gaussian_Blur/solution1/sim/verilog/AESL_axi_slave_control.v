...
//------------------------Parameter----------------------
`define TV_IN_w "../tv/cdatafile/c.Blur.autotvin_w.dat"
`define TV_IN_h "../tv/cdatafile/c.Blur.autotvin_h.dat"
...
parameter h_DEPTH = 1;
reg [31 : 0] h_OPERATE_DEPTH = 0;
parameter h_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter Blur_continue_addr = 0;
parameter Blur_auto_start_addr = 0;
parameter w_data_in_addr = 16;
parameter h_data_in_addr = 24;
parameter sigma_data_in_addr = 32;
parameter kernal_data_in_addr = 44;
...
reg [DATA_WIDTH - 1 : 0] mem_h [h_DEPTH - 1 : 0];
reg h_write_data_finish;
...
//write h reg
reg [31 : 0] write_h_count = 0;
reg write_h_run_flag = 0;
reg write_one_h_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & w_write_data_finish & h_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        h_write_data_finish <= 0;
        write_h_run_flag <= 0; 
        write_h_count = 0;
        count_operate_depth_by_bitwidth_and_depth (h_c_bitwidth, h_DEPTH, h_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            h_write_data_finish <= 0;
        ...
        if (write_one_h_data_done === 1) begin
            write_h_count = write_h_count + 1;
            if (write_h_count == h_OPERATE_DEPTH) begin
                write_h_run_flag <= 0; 
                h_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_h
    integer write_h_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] h_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = h_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_h_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write h data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (h_c_bitwidth < 32) begin
                        h_data_tmp_reg = mem_h[write_h_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < h_c_bitwidth) begin
                                h_data_tmp_reg[j] = mem_h[write_h_count][i*32 + j];
                            end
                            else begin
                                h_data_tmp_reg[j] = 0;
                            end
                    ...
                    write (h_data_in_addr + write_h_count * four_byte_num * 4 + i * 4, h_data_tmp_reg, write_h_resp);
                end
                process_busy = 0;
                write_one_h_data_done <= 1;
                @(posedge clk);
                write_one_h_data_done <= 0;
            end   
            process_2_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_h_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [h_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] mem_tmp; 
  reg [ 8*5 : 1] str;
  integer transaction_idx; 
  integer i; 
  transaction_idx = 0; 
  mem_tmp [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (h_c_bitwidth , factor);
  fp = $fopen(`TV_IN_h ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_h); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
